...
public class DoubleArrayTrie<V> implements Serializable, ITrie<V> {
	...
	while (true) {
		...
		resize(begin + siblings.get(siblings.size() - 1).code
			+ Character.MAX_VALUE);
	    }
     ...
     *            对应每个key的长度，留空动态获取
     * @param _value
     *            每个key对应的值，留空使用key的下标作为值
     * @param _keySize
     *            key的长度，应该设为_key.size
     * @return 是否出错
     */
	...
	if (!(IOAdapter == null ? loadBaseAndCheckByFileChannel(path) : load(
		ByteArrayStream.createByteArrayStream(path), value)))
	    return false;
	v = value;
	return true;
    }
	...
	try {
	    DataInputStream in = new DataInputStream(new BufferedInputStream(
		    IOAdapter == null ? new FileInputStream(path)
			    : IOAdapter.open(path)));
	    size = in.readInt();
	    base = new int[size + 65535]; // 多留一些，防止越界
	    check = new int[size + 65535];
	...
	try {
	    in = new ObjectInputStream(IOAdapter == null ? new FileInputStream(
		    path) : IOAdapter.open(path));
	...
	for (int i = pos; i < len; i++) {
	    p = b + (int) (keyChars[i]) + 1; // 状态转移 p = base[char[i-1]] +
					     // char[i] + 1
	...
	for (int i = 0; i < len; ++i) {
	    ...
	    if (b == check[p] && n < 0) // base[p] == check[p] && base[p] < 0
					// 查到一个词
	    {
		result.add(new AbstractMap.SimpleEntry<String, V>(new String(
			keyChars, 0, i), v[-n - 1]));
	    }
	...
	if (b == check[p] && n < 0) {
	    result.add(new AbstractMap.SimpleEntry<String, V>(new String(
		    keyChars, begin, len - begin), v[-n - 1]));
	}
	...
	// {
	// infoChar += "    " + (i == check[i] ? " ×" : (char)(i - check[i] -
	// 1));
	...
	return "DoubleArrayTrie{"
		...
		"size=" + size + ", allocSize=" + allocSize + ", key=" + key
		+ ", keySize="
		+ keySize
		+
		// ", length=" + Arrays.toString(length) +
		// ", value=" + Arrays.toString(value) +
		", progress=" + progress + ", nextCheckPos=" + nextCheckPos
		+ ", error_=" + error_ + '}';
    }
